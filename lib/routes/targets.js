...
function registerTargets (router) {
  ...
  router.set('api/targets', (req, res, op) => {
    // create target
    if (req.method === 'POST') return targetController.createTarget(req, res, op)

    // get targets
    if (req.method === 'GET') return targetController.getAllTargets(req, res, op)
  ...
  router.set('api/targets/:id', (req, res, op) => {
    // get target
    if (req.method === 'GET') return targetController.getTarget(req, res, op)

    // update target
    if (req.method === 'POST') return targetController.updateTarget(req, res, op)
