...
async function createTarget (req, res, options) {
  try {
    // get next seq id
    const id = await toRedisPromise(redis.INCRBY)('targets:seq_count', 1)
    const targetObj = {
      id,
    ...
    await toRedisPromise(redis.HSET)(`targets:${id}`, targetPayload)
    await toRedisPromise(redis.SADD)('targets:list', String(id))
    ...
    targetObj.accept.geoState.map((geoState) => {
      targetObj.accept.hour.map((hour) => {
        sortedSetPromises.push(
          toRedisPromise(redis.ZADD)(`route:geoState:${geoState}:hour:${hour}`, targetObj.value, targetObj.id),
          toRedisPromise(redis.SADD)(`targets:${id}:sortedSets`, `route:geoState:${geoState}:hour:${hour}`)
        )
      })
    })
...
async function getAllTargets (req, res, options) {
  try {
    const targetIds = await toRedisPromise(redis.SMEMBERS)('targets:list')
    let targets = []
    if (targetIds) {
      targets = await Promise.all(targetIds.map(s => toRedisPromise(redis.HGETALL)(`targets:${s}`)))
...
async function getTarget (req, res, options) {
  try {
    const target = await toRedisPromise(redis.HGETALL)(`targets:${options.params.id}`)
...
async function updateTarget (req, res, options) {
  try {
    ...
    await toRedisPromise(redis.HSET)(`targets:${id}`, targetPayload)
    const sortedSets = await toRedisPromise(redis.SMEMBERS)(`targets:${id}:sortedSets`)

    await Promise.all([
      ...sortedSets.map((sortedSet) => toRedisPromise(redis.ZREM)(sortedSet, id)),
      toRedisPromise(redis.DEL)(`targets:${id}:sortedSets`)
    ])
